...
def parse_turn(sess: Session, text: str):
    ...
    for name, u in updates.items():
        if name not in sess.fields or not isinstance(u, dict):
            continue
        prov = str(u.get("provenance", "unknown")).strip().lower()
        if prov not in ("stated", "inferred", "unknown"):
            continue  # an unrecognized label would otherwise bypass the HUMAN_OWNED guard
